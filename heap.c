...
/********************************************************************************
* double_array_assign: Fyller array till bredden med flyttal med godtyckligt
*                      startvärde samt stegvärde.
* 
*                      - self     : Pekare till arrayen som ska fyllas.
*                      - size     : Arrayens storlek (antalet tal den rymmer).
*                      - start_val: Startvärde, tilldelas till första index.
*                      - step_val : Stegvärde, dvs. differensen mellan varje
*                                   tilldelat flyttal.
********************************************************************************/
static void double_array_assign(double* self,
                                const size_t size,
                                const double start_val,
                                const double step_val)
{
   double num = start_val; 
...
/********************************************************************************
* double_array_print: Skriver ut flyttal lagrade i angiven array via godtycklig
...
********************************************************************************/
static void double_array_print(const double* self, 
                               const size_t size,
                               FILE* ostream)
{
...
/********************************************************************************
* main: Allokerar minne för ett fält som rymmer 20 flyttal på heapen. Ifall
...
*       med flyttal räknat från 0 med stegvärde 0.5 (0, 0.5, 1, ... 9, 9.5), 
*       följt av utskrift i terminalen.
*
*       Därefter omallokeras fältet så att det rymmer 40 flyttal i stället.
...
*       fältet med flyttal räknade från 0 med stegvärde -0.5. De heltal som
...
*       en textfil döpt "numbers.txt". Slutligen frigörs minnet som har 
*       allokerats för fältet. Dynamiskt allokerat minne måste alltid frigöras 
*       av användaren. Om detta missas så frigörs inte minnet förrän då du 
...
int main(void)
{
   ...
   double_array_assign(data, 20, 0, 0.5); 
   double_array_print(data, 20, 0); 
   double_array_print(data, 20, f1);

   double* copy = (double*)realloc(data, sizeof(double) * 40); 

   if (copy) 
   {
      data = copy; 
      double_array_assign(data + 20, 20, 0, -0.5); 
      double_array_print(data, 40, 0); 
      double_array_print(data, 40, f1); 
   }
